...
// Virtual aliases for latitude/longitude to maintain backward compatibility
coordinatesSchema.virtual("latitude").get(function () {
  return this.lat;
}).set(function (value) {
  this.lat = value;
});

coordinatesSchema.virtual("longitude").get(function () {
  return this.lng;
}).set(function (value) {
  this.lng = value;
});
...
const propertySchema = new mongoose.Schema(
  {
    title: {
      ...
      trim: true,
      minlength: [3, "Property title must contain at least 3 characters"],
    ...
    description: {
      ...
      trim: true,
      minlength: [20, "Property description must contain at least 20 characters"],
      maxlength: [3000, "Property description cannot exceed 3000 characters"],
    },
    ...
    bedrooms: {
      ...
      min: [1, "A property must have at least 1 bedroom"],
    },
    ...
    totalRooms: {
      ...
      min: [1, "A property must have at least 1 room"],
    },
    ...
    availableRooms: {
      ...
      validate: {
        validator(value) {
          if (this.totalRooms === undefined || this.totalRooms === null) {
            return true;
          }
...
propertySchema.pre("validate", function validatePropertyApproval() {
  if (this.approvalStatus === "rejected" && !this.rejectionReason?.trim()) {
    this.invalidate(
      "rejectionReason",
  ...
  if (this.isFeatured && this.approvalStatus !== "approved") {
    this.invalidate(
      "isFeatured",
...
// 2dsphere index for geolocation distance queries
propertySchema.index({
